...
export default function TopHeader() {
    ...
    return (
        <>
            <Container fluid bg='#F0F6ED' pb={smallerLgScreen ? '30px' : '80px'} >
                <Layout mt={smallerXsScreen ? '66px' : smallerLgScreen ? '80px' : '70px'} pt={smallerLgScreen ? '40px' : '90px'}>
                    <Grid.Col xs={12} ta='center'>
                        <Title order={1} color='#252D31'>{headerContent.title}</Title>
                    </Grid.Col>
                    <Grid.Col xs={12} sm={12} md={12} lg={9} mx='auto' ta='center'>
                        <Text component='p' sx={{opacity:'0.8'}} color='#252D31' my={smallerLgScreen ? 'xs' : 'xl'} >{headerContent?.description}</Text>
                    </Grid.Col>
                </Layout>
                <Search />
            </Container>
