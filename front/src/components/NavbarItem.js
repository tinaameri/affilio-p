...
const useStyles = createStyles((theme) => ({
  link: {
    ...
    ...theme.fn.hover({
      backgroundColor:
        theme.colorScheme === 'dark' ? theme.colors.dark[6] : '#F6FFF2',
      height: '85px',
    }),
  },
...
export function CardSubMenu({ item, onClick }) {
  const { classes } = useStyles();
...
export default function NavbarItem({ services, company, home }) {
  const { classes } = useStyles();
          ...
          >
            <HoverCard.Target>
              <Box
                ...
                onMouseEnter={handleHoverVisible}
              >
                <Center inline>
            ...
            {isParentVisible && (
              <HoverCard.Dropdown
                onMouseLeave={handleHoverHidden}
                sx={{
                  overflow: 'hidden',
  ...
  const singleItemMenu = (title, link) => (
    <Box
      className={`${classes.link} menu-link ${activeHome === true && 'active'}`}
      onClick={() => handleCloseHome(true)}
    >
      ...
      {company?.map((items) =>
        items?.map((menu) =>
          menu?.subMenu?.map((menu, subIndex) => (
            <Box
              className={`${classes.link} menu-link ${
                active === subIndex && 'active'
              }`}
              onClick={() => handleClose(subIndex)}
              key={subIndex}
            >
