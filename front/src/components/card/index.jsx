import { Text, Title,  Button, Box, Grid, Flex, createStyles, BackgroundImage,Image } from '@mantine/core';
...
export function CardItem(props) {
    ...
    const { src, order, color, title, description, type, btnColor, href, btns, header, sx,lottie,orderLgImg,topTitle,mobileOrder,height,pt,pb,mt,contentAlign,plContent,prContent,borderColor,imgHeight,onClick,headline,variant,btnIcon,btnBg,number,orderOdd,className,targetBlank,bg,heading_title,heading_description,imageAlt,multiText,bgSection } = props;
    ...
    return (
        ...
        <Box      
         bg={bgSection}
         sx={sx} h={height}   mt={mt} pt={pt} pb={pb} className='cover-bottom'>
             <Fade top distance='10%' duration={1000}>

                <Layout pt='45px' pb='70px' className='flex-align-center'>
                    {heading_title && <Heading title={heading_title} description={heading_description}/>}
                    ...
                    <Grid.Col xs={12} sm={12} md={12} lg={btns?.length < 2 ? 6 : 7} pt='xl' sx={{alignSelf:'center'}}>
                        <Box pl={plContent} pr={prContent} ta={smallerScreen && contentAlign}>
                       {topTitle && <Text mb='lg' component='span'  fw='500'   >{topTitle}</Text>} 
                        {number && <Text component='span' className={classes.box}  mb='xl'>{number}</Text>}
...
{multiText?.map((item,index)=>(
    <>
                        ...
                        <Title order={3>multiText?.length>1 ? 4 : multiText?.length>2 ? 5 : 3} mt='md'  fw='500'> 
                        {item?.title}
                      </Title>
...
{btns && 
 <Flex
                                h={(btns?.length === 0 ? '0' : btns?.length < 2 ? 60 : (smallerScreen ? 140 : 132))
                                }
                                w='100%'
                                gap="sm"
...
{btns?.map((button, index) => (
    <ButtonComponent type={button?.type} title={button?.title} targetBlank={button?.newPage} href={button?.link} key={index} />
))}
...
export function CardList(props) {
    ...
    const cardItems = Object.values(content)?.map((card, index) => (
        <CardItem key={index} className={props.className} onClick={props.onClick} top_header={card?.top_header} top_description={card?.top_description }  btnBg={props.btnBg} variant={props.variant} btnIcon={props.btnIcon} headline={props.headline} sx={props.sx} 
        
        // bg={(index >= 0 && index <= 1) ? props.bgEven : props.bgOdd}
        bg={card?.bg}
        color={(index >= 0 && index <= 1) ? (props.colorEven) : (props.colorOdd ?? 'white')} btnColor={ props.btnColor} number={card?.number} topTitle={card?.topTitle} title={card?.title} description={card?.desc} multiText={card?.multi_text} btns={card.buttons} src={card?.img} lottie={card?.lottie} value={card?.value} contentAlign={props.contentAlign} imgHeight={card?.imgHeight} 
            order={index % 2 == 0 ? (props.evenOrder ?? 1) : (props.oddOrder ?? 0) }
         plContent={largerMidScreen && index % 2 == 0 ? '16px' : '0'} prContent={largerMidScreen && index % 2 !== 0 ? '16px' : '0'} pt={(index % 2 == 0) ? (largerMidScreen ? '16px'
            : largerSmallScreen ? '54px'
                : smallerXsScreen ? '32px'
                    : undefined): '0'}  pb={(index % 2 !== 0) ? (largerMidScreen ? '16px'
                    : largerSmallScreen ? '54px'
                        : smallerXsScreen ? '32px'
                            : undefined): '10px'}  mobileOrder={props.mobileOrder} height={props.height} mt={props.mt}  />
