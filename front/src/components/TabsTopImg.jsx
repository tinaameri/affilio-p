...
export default function TabsTopImg({ topTitle, topDescription, tabs, imgHeight }) {
            ...
            >
                <Tabs.List grow position="center">
                    {tabsContent?.map((tab, index) => (
                        <Tabs.Tab
                            key={index} value={tab?.value} className='icon-secondary-color'>
                            <Flex
                                mih={109}
                    ...
                    <Tabs.Panel key={index} value={tabs?.value} pt="xs" id={tabs?.value}>
                        {tabs?.header?.map((item, idx) => (
                            <Box key={idx} mb='xl' pt='lg' >
                                <Title color='primary.1' order={3} py='xl'>{item?.title}</Title>
                        ...
                        <Grid py='xl'>
                            ...
                            <Grid.Col sm={12} xs={12} md={12} lg={6} xl={6} >
                                {tabs?.header?.map((item, index) => (
                                    <Box key={index} pos='relative' h={imgHeight ?? '538px'} w='100%'>
                                        <Image
                                            layout='fill'
                                            objectFit='contain'
                                            src={item?.img}
                            ...
                            <Grid.Col sm={12} >
                                ...
                                <Box pb="80px">
                                    <Heading  title={header} headline={3} color='primary.1' py='40px' />
