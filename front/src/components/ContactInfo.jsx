...
import { Container, Grid, Title, Box, Flex, Text, List, ThemeIcon, Image } from '@mantine/core';
...
export default function ContactInfo({ content }) {
  ...
  return (
    ...
    <Container fluid 
      ...
      <Layout pt='100px'>
        <Heading py='70px' color='white' bg='primary.1' title={content?.title} description={content?.description} />
        <Grid.Col xs={12} sm={12} md={12} lg={6} xl={6} mx='auto' pt={smallerLgScreen ? '0' : '90px'}
        ...
        >
          <Fade right>
            <>
                    ...
                    <Box key={index} mb='xl'>
                      ...
                      <Box mt='40px'>
                          <Flex align='start' key={index} mt='xl'>
                            ...
                            <Text component='a'  fw='400' fz='md' target='_blank' title={item?.info} href={item?.link} >
                              {item?.info}
                            </Text>
                            {/* </Link> */}
                          </Flex>
              ...
              <Text component='strong' ff='Yekan Bakh Bold' fz='xl' fw='900' mb='xl'>
        ...
        <Grid.Col xs={12} sm={12} md={10} lg={6} xl={6} mx='auto' ta='center' order={smallerLgScreen && 1}>
          <Fade left>
            <Image src={`${IMAGES_BASE_UR}${content?.image?.data?.attributes?.url}`} height={content?.image?.data?.attributes?.height} width='100%' withPlaceholder fit='contain' alt='contact-us' />
          </Fade>
        </Grid.Col>
      </Layout>
