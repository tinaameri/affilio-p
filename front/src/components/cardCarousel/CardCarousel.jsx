...
const CardCarousel = ({ children, cardItems }) => {
...
export function Card({ item }) {
  return (
    <Paper size='xl' w='100%' radius="lg"  >
      <Image src={`${IMAGES}/home/testimonial.svg`} width={45} height={40} alt='testimonial' mb='30px' />
      ...
      <Flex align='center' mt='xl'>
        <Box mr='xl' sx={{ borderRadius: '50%' }} className='flex-center' bg='gray.0' w='114px' h='114px'>
          <Image width='100%' p='xl' fit='contain' src={`${IMAGES_BASE_UR}${item?.image?.data?.attributes?.url}`} alt={item?.full_name} />


        </Box>
        <Box>
          <Text component='strong' fw='900' fz='lg' color='rgba(14, 14, 14, 0.87)' mt='md'>
            {item?.full_name}
          </Text>
